...
class YoutubeTrackRepository:
    ...
    def total_of_tracks(self, channel_id):
        return (
            self.session.query(func.count("*"))
            .select_from(YoutubeTrack)
    ...
    def get_total_of_views(self, channel_id):
        return (
            self.session.query(func.sum(YoutubeTrack.views))
            .filter(
                and_(
    ...
    def get_average_of_views(self, channel_id):
        return (
            self.session.query(func.avg(YoutubeTrack.views))
            .filter(
                and_(
                    YoutubeTrack.channel_id == channel_id,
                    YoutubeTrack.views.is_not(None),
    ...
    def get_total_of_likes(self, channel_id):
        return (
            self.session.query(func.sum(YoutubeTrack.like_count))
            .filter(
                and_(
                    YoutubeTrack.channel_id == channel_id,
    ...
    def get_top_10_tracks(self, channel_id):
        return (
            self.session.query(YoutubeTrack)
            .filter(
                and_(
                    YoutubeTrack.channel_id == channel_id,
                    YoutubeTrack.views.is_not(None),
            ...
            .order_by(YoutubeTrack.views.desc())
            .limit(10)
            .all()
        )
    ...
    def insert_tracks(self, channel_id):
        ...
        raw_insert = postgresql.insert(YoutubeTrack).values(
            [track.to_dict() for track in tracks_by_id.values()]
        )
        stmt = raw_insert.on_conflict_do_update(
            index_elements=["id"],
            set_={
                k: raw_insert.excluded[k.name]
                for k in YoutubeTrack.__table__.columns
                if k.name != "id"
            },
        )
